...
<body class="">
                    ...
                    <div class="card table-card">
                        <div class="card-header text-center">
                            <div class="row m-b-25 align-items-center justify-content-center">
                                ...
                                <div>
                                    <a href="#!">
                                        <h6 class="m-b-0">Catalogo de Productos y Servicios</h6>
                                    </a>
                                </div>
                            </div>
                        </div>
                ...
                <div class="modal-body">
                    <form id="formProductos">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" id="ingreso" min="1">
                        </div>
                        <div class="form-group">
                            <label for="fIngreso">Fecha de ingreso</label>
                            <input type="date" class="form-control" id="fIngreso" min="1">
                        </div>
                        <div class="form-group">
                            <label for="cantidadAlmacen">Existía en almacén</label>
                            <input type="number" class="form-control" id="cantidadAlmacen" min="1">
                        </div>
                        <div class="form-group">
                            <label for="cantidad">Total en almacén</label>
                            <input type="number" class="form-control" id="cantidad" placeholder="Cantidad en almacén" step="0.5" min="1">
                        </div>
                        <div class="form-group">
    ...
    <script>
        $("body").append('<div class="fixed-button active btn btn-md btn-info" style="background-color:#7267ef;border-radius:50%;"><i class="fa fa-plus btn_nuevo" aria-hidden="true" data-toggle="modal" data-target="#staticBackdrop" id="abrirModal"></i></div>');
    </script>
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#ingreso, #cantidadAlmacen").on("input", function() {
                let totalAlmacen = parseInt($("#ingreso").val()) + parseInt($("#cantidadAlmacen").val());
                $("#cantidad").val(totalAlmacen);
            });
            ...
            */
            ...
            tablaProductos = $("#productosCat").DataTable({
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            return `<label class="badge badge-light-dark">${data}</label>`;
                        }
                    },
                    {
                        data: "compra"
                ...
                rowCallback: function(row, data) {
                    ...
                    if (data['cantidad'] <= 3 && data['categoria'] != 'SERVICIO') {
                        ...
                        $($(row).find("td")[11]).css("color", "#B72949");
                        ...
                        $($(row).find("td")[11]).css("color", "#2980B9");
                        $($(row).find("td")[11]).css("font-weight", "500");
                    }
            ...
            $(document).on("click", ".btnEditar", function() {
                ...
                let p_d = parseInt(fila.find("td:eq(7)").text());
                let ingreso = parseInt(fila.find("td:eq(8)").text());
                let fechaIngreso = fila.find("td:eq(9)").text();
                let almacen = parseInt(fila.find("td:eq(10)").text());
                let cantidad = fila.find("td:eq(11)").text();
                let categoria = fila.find("td:eq(12)").text();
                ...
                $("#compra").val(compra);
                $("#precio").val(precio);
                $("#descuento").val(calcularPorcentaje(p_d));
                $("#p_d").val(p_d);
                $("#ingreso").val(ingreso);
                $("#fIngreso").val(fechaFormateada);
                $("#cantidadAlmacen").val(almacen);
                ...
                $("#actualizarProducto").click(function() {
                    ...
                    let ingreso = $("#ingreso").val();
                    let fechaIngreso = $("#fIngreso").val();
                    let almacen = $("#cantidadAlmacen").val();
                    let cantidad = $("#cantidad").val();
